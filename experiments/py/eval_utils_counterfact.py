...
def test_batch_prediction(
    ...
    # n_tok = [tok(f" {n}", add_special_tokens=False)["input_ids"] for n in neighborhood_true]
    n_tok = neighborhood_true
    ...
    for i in range(logits.size(0)):
        if i > logits.size(0) - 2 * len(n_tok):
            n_ids = (i - logits.size(0) + 2 * len(n_tok) - 1) // 2
            b_tok = n_tok[n_ids]
